...
class FiniteField():
    ...
    Below implements an elementary finite field class with finite field arithmetic.

    The polynomials are represented by lists of coefficients, highest degrees first.
    For example, the elements of F_{3^2} are:
    ...
    def field(self) -> list:
        ...
        """
        return [np.trim_zeros(list(c), trim='f') for c in itertools.product(range(self.p), repeat=self.n)]
    ...
    def power(self, x: list, k: int) -> list:
        ...
        """
        if k < 0:
            raise ValueError('Exponent must be a nonnegative integer, not {:s}'.format(k))
        elif k == 0:
            return self.neutral
        else:
    ...
    def legendre_symbol(self, x: list) -> int:
        ...
        (x | F_q) = 1 if x is a quadratic residue in F_q^*, -1 if not, and 0 if x = 0.
        Euler's criterion (or simply counting the number of quadratic residues and applying
        Lagrange theorem on subgroup cardinals) implies that (x | F_q) = x ^ ((q-1)/2).
        """
        if len(x) == 0:
            return 0
        else:
            ...
            if len(symbol) > 1:
                raise Exception("Euler's criterion failed ({}^{} is of degree > 1)".format(x, (self.q-1)//2))
            else:
                if int(symbol[0]) == 1:
                    return 1
                    ...
                    raise Exception("Euler's criterion failed ({}^{} is not -1, 0 or 1)".format(x, (self.q-1)//2))
